...
- **`system/supervisord.conf`** carries the `kid-events-radar` program entry.
  ...
  (`--url http://localhost:8080 --name kid-events-radar`) and then runs
  `uv run kid-events-radar`, so the tab is served on port 8080 and supervised
...
### v1 (2026-08-31) -- first release: the kid-events pipeline skill plus the Family Weekend Radar web app, with a neutral San Francisco default home and a guided setup for adapting the source list to any city.
